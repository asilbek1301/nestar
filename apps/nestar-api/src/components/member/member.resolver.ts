...
export class MemberResolver {
	...
	public async imageUploader(
		...
		@Args('target') target: String, // image qayerga saqlanishi
	): Promise<string> {
		...
		const imageName = getSerialForImage(filename); // making random name for image
		...
		const result = await new Promise((resolve, reject) => {
			stream
				...
				.on('finish', async () => resolve(true)) // upload successfully
				.on('error', () => reject(false)); // upload error
		});
